...
CROWN_SPARE: dict[PoleType11, dict[tuple[str, str], int]] = {
    ...
    PoleType11.LATTICE: {M_DISC_INSULATOR_11: 2, M_PIN_INSULATOR_11: 1},
}
"""احتياط التاج: بدلُ العازل القائم على رأس العمود إذا تلف عند نقله (ق-٨٤).

**الأرقام الوحيدة المكتوبة في هذا الملف**، لأنها بدلُ تالفٍ لا حاجةُ دائرةٍ
جديدة — فلا يعرفها مولّد الشبكة.

| العمود | البدل | المجموع مع فرق الدائرة |
|---|---|---|
| مدوّر | +1 دبوسي | **4** دبوسي |
| مشبك | +1 دبوسي **و**+2 قرصي | **4** دبوسي و**8** قرصي |

**والدبوسي للنوعين معاً:** لكل عمودٍ تاجٌ يُنقل، مشبكاً كان أو مدوّراً — أقرّه
المستخدم صراحةً: «من الأفضل إضافة عازل دبوسي (مع السبندل) واحد لكل عمود مشبك
(بسبب التاج) لتكون 4×9». **ومعدات الربط بلا بدل.**
"""
