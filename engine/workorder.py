...
class WorkOrder:
    ...
    material_overrides: dict[str, float] = field(default_factory=dict)
    """كميات مواد عدّلها المستخدم يدوياً: «الاسم|الوحدة» ← الكمية (ق-٨٢).

    **والمحسوب لا يُحفَظ معها**: ملف `.wo` يحمل المُدخَلات وحدها ويُعاد الحساب
    عند كل فتح (ق-٦١)، فالتراجع يبقى ممكناً بعد شهور وعلى حاسبة أخرى.
    """

    labour_overrides: dict[str, float] = field(default_factory=dict)
    """نظيرها لبنود الأجور."""
