...
class LabourLine:
    ...
    manual: bool = False
    """هل جاءت كميته من تعديل يدوي لا من الحساب؟ (ق-٨٢)"""

    computed_qty: float | None = None
    """الكمية المحسوبة قبل التعديل اليدوي — تُعرض بجانبه فلا يلتبس الرقمان."""

    driver: tuple[str, str] | None = None
    """(اسم المادة، وحدتها) التي **تقود كمية هذا البند** — أو None (ق-٨١).

    أجر التسليك كميته أمتار السلك نفسها، وأجر نصب العمود عددُ الأعمدة نفسه.
    فهذا الحقل يُعلن تلك العلاقة **صراحةً حيث تُعرف**، أي في المحرك.

    **ولماذا إعلانٌ لا استنتاج:** لو رُبط بتطابق الأرقام لوقع خطأ مقيس — في
    مشروع حقيقي كانت كمية «عبور الشوارع الفرعية» 24 وكمية «ترمنل 50 ملم²» 24
    كذلك، **مصادفةً**. فربط الأرقام كان سيجعل تعديل الترمنلات يغيّر أجر عبور
    الشوارع.

    ولا يغيّر هذا الحقل حساباً: هو **وصفٌ** لعلاقة قائمة، يقرأه التصدير إلى
    إكسل ليجعل الكمية معادلةً، ويقرأه التعديل اليدوي ليُسري تعديل المادة على
    أجرها.
    """
...
class Equipment:
    ...
    كل تجهيز يجرّ خلفه مجموعة ملحقات ثابتة (المحولة 15 مادة، الفاصل هوائي 11 ك.ف ON LOAD على
    رأس القابلو 9 مواد …)، وأجر عمله يشمل ملحقاته كلها فلا أجر مستقل لها (ق-٢٣).
    """

    transformers: dict = field(default_factory=dict)
    """عدد المحولات: {(TransformerVoltage, TransformerSize): العدد}.

    المفتاح **ثنائي**: الجهد التحويلي ثم السعة (ق-٣٧). لا يكفي مفتاح السعة وحدها
    لأن محولة 400 KVA جهد 11/0.4 ومحولة 400 KVA جهد 33/0.4 مادتان مختلفتان
    بسعرين مختلفين وبمانعة صواعق ولنك فيوز مختلفين.

    السعات المتاحة: 11/0.4 ← 250 و400 و630، و33/0.4 ← 400 و630 فقط (ق-٣٧)."""

    onload_11_mid: int = 0
    """فاصل هوائي 11 ك.ف ON LOAD جهد 11 ك.ف في منتصف الشبكة — بلا مانعة صواعق (ق-٢٥)."""

    onload_11_head: int = 0
    """فاصل هوائي 11 ك.ف ON LOAD جهد 11 ك.ف على رأس القابلو — مع مانعة، ونصف كمية القابلو."""

    isolator_33_mid: int = 0
    """فاصل هوائي 33 ك.ف ON LOAD في منتصف الشبكة — بلا مانعة صواعق."""

    isolator_33_head: int = 0
    """فاصل هوائي 33 ك.ف ON LOAD على رأس القابلو — مع مانعة، ونصف كمية القابلو."""
