...
M_EARTH_ROD = ("قضيب نحاس تأريض 1.5 متر مع القفيص", "عدد")
...
"""ترمنل 150 ملم² لكل مخرج. مخرجان ← **24**، وأربعة ← 48 (ق-٤٣، وأكّدتَ الـ24 في ق-٥٦).
...
AL_FITTINGS_CU_PER_TRANSFORMER = 13
"""معدات ربط ألمنيوم – نحاس لكل محولة — **13 ثابتة لا تتبع المخارج** (ق-٥٦).

كانت 4 لكل مخرج (8 لمخرجين) في ق-٤٣، فصحّحتَها إلى **13 لكل محولة**. والرقم
فردي لا يقبل القسمة على المخارج، فهو ثابت لا نسبة."""

EARTH_TERMINAL_PER_TRANSFORMER = 15
"""ترمنل 50 ملم² لكل محولة — **ثابت لا يتبع المخارج** (ق-٤٣، والعدد 15 بـ ق-٥٦).
...
TRANSFORMER_COMMON_KIT = [
    ...
    (M_AL_FITTINGS_CU, AL_FITTINGS_CU_PER_TRANSFORMER),
]
...
def transformer_kit(
    ...
    **قاطع الدورة لا يتبع الجهد** — الضغط الواطئ 0.4 ك.ف في الحالتين.
    ...
    return [
        ...
        # بندان يتبعان عدد المخارج: القابلو وترمنله (ق-٤٣؛ ومعدات الربط
        # خرجت منهما إلى الثابت بـ ق-٥٦)
