...
def _material_rows(html_text: str) -> list[str]:
    """أسطر بيانات جدول المواد وحده — من عنوان القسم أ إلى نهاية جدوله.

    **الحدّ الأدنى صار `</table>` لا عنوانَ القسم ب (ق-٦٨):** الجداول صارت
    متجاورة داخل جدول تخطيط، وترتيب الخلايا مقلوب، فقسم «ب» يسبق قسم «أ» في
    نصّ HTML وإن ظهر يساره في الورقة.

    والاستبعاد بـ (?!<th) يُسقط سطر العناوين.
    """
    section = html_text.split("أ - جدول المواد المخمنة")[1].split("</table>")[0]
